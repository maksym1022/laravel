@extends('templates.main')
    ...
    <legend>Edit Submenu</legend>
    <ul>
        @foreach($errors->all() as $error)
            <li>{{ $error }}</li>
        @endforeach
    </ul>    
    {{ Form::open(array('url' => 'submenus/save/'.$menus->id,'class'=>'form-horizontal') ) }}    
    
        <div class="form-group">
            {{ Form::label('menu_id','Menu:', array('class' => 'col-md-2 control-label'))}}
            <div class="col-md-3">
            
            {{ Form::select('menu_id',$menu_lists,$menu_id, ['class' => 'form-control'])}}
            </div>
        </div>
        <div class="form-group">
            {{ Form::label('name', 'Name:', array('class' => 'col-md-2 control-label'))}}
            <div class="col-md-3">
            {{ Form::text('titulo', $menus->titulo,array('class' => 'form-control'))}}
            </div>
        </div> 
        <div class="form-group">
            {{ Form::label('link', 'Link:', array('class' => 'col-md-2 control-label'))}}
            <div class="col-md-3">
            {{ Form::text('link', $menus->link,array('class' => 'form-control'))}}
            </div>
        </div>  
        <div class="form-group">
            {{ Form::label('action','Actions: ', array('class' => 'col-md-2 control-label', 'for' => 'editar-element-6' )) }}                       
            <div class="col-md-6">
            {{ Form::select('actions[]', $actions, $id_action, array('class' => 'chosen-select form-control', 'multiple' => 'multiple', 'data-placeholder' => 'Choose actions...' ))}}
            </div>
        </div>
