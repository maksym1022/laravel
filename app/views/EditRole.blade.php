<?php 
    ...
    foreach($relations as $relation){    	   
       ...
       if(!is_null($submenu_id) && !is_null($menu_id)){                                                                                   
    		...
    		$options[] .='<option '.((isset($permissions[$id])) ? 'selected="selected"' : '').' value="'.$id.'">'.$permission.'->'.SubMenus::find($submenu_id)->titulo.((!is_null($action_id)) ? '->'.$slug_actions->slug : '').'</option>';                                        
    	}elseif(!is_null($menu_id) && is_null($submenu_id)){
    		$menus_titulo = DB::table('menus')->where('id', $menu_id)->first();  
            $options[] .='<option '.((isset($permissions[$id])) ? 'selected="selected"' : '').' value="'.$id.'">'.$menus_titulo->titulo.((!is_null($action_id)) ? '->'.$slug_actions->slug : '').'</option>';                
    	}              
    }
...
@extends('templates.main')
    ...
    {{ Form::open(array('url' => 'roles/save/'.$role->id,'class'=>'form-horizontal') ) }}        
        ...
        <div class="form-group">        
            ...
            <div class="col-md-6">
            {{ Form::select('roles[]', $roles, $selected, array('class' => 'chosen-select form-control', 'multiple' => 'multiple', 'data-placeholder' => 'Choose roles...' ))}}
            </div>
        </div>
       ...
       <div class="form-group">
			<label class="col-md-2 control-label" for="editRole-element-4">
				<span class="required">* </span>General Data:
			</label>
			<div class="col-md-3">
				<select name="general" required="" id="editRole-element-4" class="form-control">
					<option value="{{$role->general}}">{{($role->general == 0 ? 'No' : 'Yes')}}</option>
